...
func (api *ResourcesApi) Upload() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		api.resources.Create(user, uuid, handler.Filename, filepath, handler.Header.Get("content-type"), written)
	})
}
...
func (api *ResourcesApi) GetAll() http.Handler {
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		user, ok := r.Context().Value(userContextKey).(*repository.User)
		if !ok {
			helper.ReturnErr(w, repository.ErrMissingUserContext, http.StatusInternalServerError)
		...
		files, metadata, err := api.resources.GetAll(user, filters)
		if err != nil {
