	...
	for {
		...
		case "STARTTLS":
			...
			if tlsConn.ConnectionState().PeerCertificates != nil {
				...
				hash := sha256.Sum256(tlsConn.ConnectionState().PeerCertificates[0].Raw) // RawTBSCertificate
				log.Printf("%s[%v]:%s %v%s\n", ColorGreen, subject, ColorGreen, fmt.Sprintf("%x", hash), ColorReset)
			}
		...
		case "AUTH":
			...
			case "OAUTHBEARER":
				s.authenticated, err = s.handleAuthOAuthBearer(authArgs)
			}
...
func (s *session) authMechs() (mechs map[string]bool) {
	mechs = map[string]bool{"LOGIN": s.tls, "PLAIN": s.tls, "OAUTHBEARER": s.tls, "CRAM-MD5": true}
...
func (s *session) handleAuthOAuthBearer(arg string) (bool, error) {
	var err error

	// If fast mode (AUTH OAUTHBEARER [arg]) is not used, prompt for the token.
	if arg == "" {
		s.writef("334 ")
		arg, err = s.readLine()
		if err != nil {
	...
	parts := bytes.Split(data, []byte{1})
	if len(parts) != 6 {
		return false, errors.New("501 5.5.2 Syntax error (unable to parse)")
	}

	log.Printf("%s%s %s %s %s%s\n", ColorCyan, parts[0], parts[1], parts[2], parts[3], ColorReset)

	// Validate token.
	authenticated, err := s.srv.AuthHandler(s.conn.RemoteAddr(), "OAUTHBEARER", parts[0], nil, nil)
